...
void funcion(int th){
	...
	while (indice_trabajo < n_elementos){
		...
		long long mi_indice_trabajo_ini = mi_fetch_and_add(&indice_trabajo, trozos);

		long long mi_indice_trabajo_fin = std::min(mi_indice_trabajo_ini + trozos, n_elementos);
