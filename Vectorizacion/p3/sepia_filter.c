...
sepia_filter_SOA0(image_t * restrict image_in, image_t * restrict image_out)
{
    ...
    for (int it=0; it < NITER; it++)
    {
        ...
        for (int i = 0; i < height*width; i++)
        {
            R_out[i] = (unsigned char) (fminf(MAXPIXV,
                          0.5f +
                          sepia[0][0]*R_in[i] + 
                          sepia[0][1]*G_in[i] + 
                          sepia[0][2]*B_in[i]));
            G_out[i] = (unsigned char) (fminf(MAXPIXV,
                          0.5f +
                          sepia[1][0]*R_in[i] + 
                          sepia[1][1]*G_in[i] + 
                          sepia[1][2]*B_in[i]));
            B_out[i] = (unsigned char) (fminf(MAXPIXV,
                          0.5f +
                          sepia[2][0]*R_in[i] + 
                          sepia[2][1]*G_in[i] + 
                          sepia[2][2]*B_in[i]));
    ...
    for (int i=0; i < height*width; i++)
    {
        pixels_out[3*i + 0] = R_out[i];
        pixels_out[3*i + 1] = G_out[i]; 
        pixels_out[3*i + 2] = B_out[i];
    }
...
sepia_filter_block(image_t * restrict image_in, image_t * restrict image_out)
{
    ...
    for (int it = 0; it < NITER; it++)
    {
        for (int i = 0; i < height*width; i+= BLOCK)
        {
            ...
            pixels_out = &image_out->pixels[3*i];
            ...
            for (int j = 0; j < BLOCK; j++)
            {
            R_out[j] = (unsigned char) (fminf(MAXPIXV,
                          0.5f +
                          sepia[0][0]*R_in[j] + 
                          sepia[0][1]*G_in[j] + 
                          sepia[0][2]*B_in[j]));
            G_out[j] = (unsigned char) (fminf(MAXPIXV,
                          0.5f +
                          sepia[1][0]*R_in[j] + 
                          sepia[1][1]*G_in[j] + 
                          sepia[1][2]*B_in[j]));
            B_out[j] = (unsigned char) (fminf(MAXPIXV,
                          0.5f +
                          sepia[2][0]*R_in[j] + 
                          sepia[2][1]*G_in[j] + 
                          sepia[2][2]*B_in[j]));
            ...
            for (int j = 0; j < BLOCK; j++)
            {
                pixels_out[3*j + 0] = R_out[j];
                pixels_out[3*j + 1] = G_out[j]; 
                pixels_out[3*j + 2] = B_out[j];
            }
        }
        dummy(image_in, image_out);
    }
